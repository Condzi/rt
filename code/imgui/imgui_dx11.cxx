...
//  ImTextureID. Read the FAQ about ImTextureID! [X] Renderer: Large meshes support
...
//  2021-06-29: Reorganized backend to pull data from a single structure to facilitate
//  usage with multiple-contexts (all g_XXXX access changed to bd->XXXX). 2021-05-19:
//  DirectX11: Replaced direct access to ImDrawCmd::TextureId with a call to
//  ImDrawCmd::GetTexID(). (will become a requirement) 2021-02-18: DirectX11: Change
//  blending equation to preserve alpha in output buffer. 2019-08-01: DirectX11: Fixed
//  code querying the Geometry Shader state (would generally error with Debug layer
//  enabled). 2019-07-21: DirectX11: Backup, clear and restore Geometry Shader is any
//  is bound when calling ImGui_ImplDX10_RenderDrawData. Clearing Hull/Domain/Compute
//  shaders without backup/restore. 2019-05-29: DirectX11: Added support for large
//  mesh (64K+ vertices), enable ImGuiBackendFlags_RendererHasVtxOffset flag.
//  2019-04-30: DirectX11: Added support for special ImDrawCallback_ResetRenderState
//  callback to reset render state. 2018-12-03: Misc: Added #pragma comment statement
//  to automatically link with d3dcompiler.lib when using D3DCompile(). 2018-11-30:
//  Misc: Setting up io.BackendRendererName so it can be displayed in the About
//  Window. 2018-08-01: DirectX11: Querying for IDXGIFactory instead of IDXGIFactory1
//  to increase compatibility. 2018-07-13: DirectX11: Fixed unreleased resources in
//  Init and Shutdown functions. 2018-06-08: Misc: Extracted imgui_impl_dx11.cpp/.h
//  away from the old combined DX11+Win32 example. 2018-06-08: DirectX11: Use
//  draw_data->DisplayPos and draw_data->DisplaySize to setup projection matrix and
//  clipping rectangle. 2018-02-16: Misc: Obsoleted the io.RenderDrawListsFn callback
//  and exposed ImGui_ImplDX11_RenderDrawData() in the .h file so you can call it
...
//  themselves. 2016-05-07: DirectX11: Disabling depth-write.
...
ImGui_ImplDX11_SetupRenderState(ImDrawData *draw_data, ID3D11DeviceContext *ctx) {
  ...
  ctx->HSSetShader(nullptr, nullptr, 0); // In theory we should backup and restore
                                         // this as well.. very infrequently used..
  ctx->DSSetShader(nullptr, nullptr, 0); // In theory we should backup and restore
                                         // this as well.. very infrequently used..
  ctx->CSSetShader(nullptr, nullptr, 0); // In theory we should backup and restore
                                         // this as well.. very infrequently used..
...
ImGui_ImplDX11_RenderDrawData(ImDrawData *draw_data) {
  ...
  // Backup DX state that will be modified to restore it afterwards (unfortunately
  // this is very ugly looking and verbose. Close your eyes!)
...
ImGui_ImplDX11_CreateDeviceObjects() {
  ...
  // By using D3DCompile() from <d3dcompiler.h> / d3dcompiler.lib, we introduce a
  // dependency to a given version of d3dcompiler_XX.dll (see D3DCOMPILER_DLL_A) If
  // you would like to use this DX11 sample code but remove this dependency you can:
  //  1) compile once, save the compiled shader blobs into a file or source code and
  //  pass them to CreateVertexShader()/CreatePixelShader() [preferred solution] 2)
  //  use code to detect any version of the DLL and grab a pointer to D3DCompile from
  //  the DLL.
  ...
  // Create the vertex shader
  {
    ...
    if (FAILED(D3DCompile(vertexShader,
                          ...
                          &vertexShaderBlob,
                          nullptr)))
      return false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error
...
ImGui_ImplDX11_InvalidateDeviceObjects() {
  ...
  } // We copied data->pFontTextureView to io.Fonts->TexID so let's clear that as
    // well.
  if (bd->pIB) {
    bd->pIB->Release();
    bd->pIB = nullptr;
